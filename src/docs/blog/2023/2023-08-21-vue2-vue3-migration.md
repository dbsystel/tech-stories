...
>
> **_[Tweet from @vuejs on Twitter](https://twitter.com/vuejs/status/1490592213184573441?s=20&t=sXiVzjkn_9nfgrYgrBK8SQ)_**
...
This means there will be no fixes and features provided anymore (unless you are [actively extending the support](https://v2.vuejs.org/lts/))
...
I am working at [DB Systel GmbH](https://www.dbsystel.de/dbsystel-en), in a DevOps Team building a Business-to-Government (B2G) solution together with our partner [Deutsche Bahn Connect GmbH](https://www.deutschebahnconnect.com) named [**_DB Curbside Management_**](https://www.deutschebahnconnect.com/en/products/curbside-management).
...
My team added the migration to Vue 3 with the new default setup and tools using [Vite](https://vitejs.dev), [Pinia](https://pinia.vuejs.org) and [Vitest](https://vitest.dev) to our backlog many months ago, but the switch to Vue 3 as default gave us another push for facing the migration.
...
- Official Vue 2 to Vue 3 [Migration Guide](https://v3-migration.vuejs.org)
- Blogpost: ["Vue.js: How to Migrate a large project from Vue 2 to Vue 3"](https://crisp.chat/blog/vuejs-migration) from [Baptiste Jamin](https://twitter.com/baptistejamin)
- Official Vue 3 notes about the [Composition API](https://vuejs.org/guide/extras/composition-api-faq.html#what-is-composition-api) and the relationship / differences compared to the Options API
- Vue Master Blog-Series by Andy Li [Part 1](https://www.vuemastery.com/blog/vue-3-migration-build) | [Part 2](https://www.vuemastery.com/blog/migration/)
- Free Vue Mastery Video Course ["From Vue 2 to Vue 3"](https://www.vuemastery.com/courses/from-vue2-to-vue3/from-vue-2-to-vue-3)
...
Check out the [official announcement](https://blog.vuejs.org/posts/vue-2-7-naruto.html) and start migrating to the Vue 3 flavour in your Vue 2 app:

#### TypeScript or not?

Are you using TypeScript right now or do you plan to migrate to TypeScript?
In that case you should read the [TypeScript Notes for Vue 3](https://vuejs.org/guide/typescript/overview.html).
...
In my previous project we weren't able to update to Vue 3 a long time since we had a dependency to [BootstrapVue](https://bootstrap-vue.org) which wasn't working with Vue 3 and isn't still.
...
- You can contribute to the dependency and help to make it Vue 3 compatible. You may have to check [VueDemi](https://github.com/vueuse/vue-demi) which is a great developing utility to create or update Universal Vue Libraries for Vue 2 & 3.
...
- We don't need a specific SCSS/SASS/LESS configuration anymore as Vite brings [support for this out-of-the-box](https://vitejs.dev/guide/features.html#css-pre-processors)
- We needed to migrate the `webpack-license-plugin` to `rollup-plugin-license` (see above)
- Vite comes with its own [approach of reading and passing environment variables and build modes](https://vitejs.dev/guide/env-and-mode.html) which is quite easy and handy
- [Static Asset Handling by Vite](https://vitejs.dev/guide/assets.html) is something you should probably know before
...
In our team we had already lots of our dependencies updated, since we are using [Mend (formerly Whitesource) Renovate](https://www.mend.io/free-developer-tools/renovate/) for housekeeping and continuous dependency version updates.
...
In our case we decided some years ago, we want to use [Leaflet.js](https://leafletjs.com/) as our map library to display and interact with features on a map.
Therefore we also used a wrapper for Vue 2 applications called [Vue2Leaflet](https://github.com/vue-leaflet/Vue2Leaflet) which made us use Leaflet in a declarative manner.
However, this architectural decision was now a problem for us, as not only this dependency is not supposed to use it with Vue 3 but also extensions for this library such as [Leaflet.heat](https://www.npmjs.com/package/@ozangokhanhergul/leaflet.heat) needed to be migrated.
...
After doing a Proof-of-Concept (PoC), we decided to switch to [OpenLayers](https://openlayers.org/) and make use of the [vue3-openlayers](https://github.com/MelihAltintas/vue3-openlayers) wrapper too, where we were also able to contribute missing features back into the project.
...
Therefore, we created the components and defined their props and events with the help of [Storybook](https://storybook.js.org/).
...
For that, you should install Vite and use the official plugin [@vitejs/plugin-vue2](https://www.npmjs.com/package/@vitejs/plugin-vue2).
...
During the migration we noticed, that we haven't used [Type-Only Imports](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-3-8.html#type-only-imports-and-export) in all our typescript and `.vue` files.
...
We had the option to either deactivate this strict behavior by setting the [typescript config option `importsNotUsedAsValues`](https://www.typescriptlang.org/tsconfig#importsNotUsedAsValues) to either `preserve` or `remove` (not recommended) or to migrate.
Luckily, there is a community project called [ts-import-types-cli](https://github.com/JamieMason/ts-import-types-cli) that will automate a part of this step.
...
In comparison to [Jest](https://jestjs.io/) it comes with a stable out-of-the-box ESM support and faster test executions.
Until now Jest's support for ESM is [still experimental](https://jestjs.io/docs/ecmascript-modules) (State: Jest Version 29.5).
[The API is quite similar and mostly compatible to jest](https://vitest.dev/guide/migration.html#migrating-from-jest).
...
At the [Pinia-Docs you will find a very detailed Guide for the Migration from Vuex](https://pinia.vuejs.org/cookbook/migration-vuex.html)

#### Migrate Components

Last but not least we decided to migrate all our components to the [composition API](https://vuejs.org/api/composition-api-setup.html) with the [`<script setup>` syntactical sugar](https://vuejs.org/api/sfc-script-setup.html).
...
Here we started by adding Vue 3 as well as the `@vue/compat` package as described in the [Vue 3 Migration Build documentation](https://v3-migration.vuejs.org/migration-build.html#migration-build).
...
- ["Vue.js: How to Migrate a large project from Vue 2 to Vue 3"](https://crisp.chat/blog/vuejs-migration) from [Baptiste Jamin](https://twitter.com/baptistejamin)
...
The migration should be straight-forward when [following the migration guide](https://test-utils.vuejs.org/migration/).
...
Now that we are using Vue 3, we can use the ["Teleport"](https://vuejs.org/guide/built-ins/teleport.html) feature.
